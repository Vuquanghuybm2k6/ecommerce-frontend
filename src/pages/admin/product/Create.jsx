...
import { Typography, Form, Input, InputNumber, Select, Switch, TreeSelect, Button, Space, Tag, AutoComplete } from 'antd'
...
function CreateProduct() {
  ...
  const handleSubmit = async (values) => {
    ...
    const processedVariants = variants.map(v => {
      const { thumbnailFile: _f, thumbnailPreview: _p, attrValues, ...rest } = v
      ...
      return {
        ...
        sku: rest.sku || `var-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      }
    })
    variants.forEach((v) => {
      if (v.thumbnailFile) {
